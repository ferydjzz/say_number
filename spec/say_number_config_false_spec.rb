  ...
  context 'use_decimal = false' do
    ...
    context 'Indonesia' do
      ...
      it 'return sepuluh miliar seratus satu juta seratus satu ribu seratus satu' do
        ...
        expect(SayNumber.say(number)).to eq 'sepuluh miliar seratus satu juta seratus satu ribu seratus satu'
      end
      it 'return satu miliar dua ratus tiga puluh empat juta lima ratus enam puluh tujuh ribu delapan ratus sembilan puluh' do
        ...
        expect(SayNumber.say(number)).to eq 'satu miliar dua ratus tiga puluh empat juta lima ratus enam puluh tujuh ribu delapan ratus sembilan puluh'
      end
      it 'return sepuluh miliar dua juta tiga ribu satu' do
        number = 10002003001.00012
        expect(SayNumber.say(number)).to eq 'sepuluh miliar dua juta tiga ribu satu'
      end
    end
